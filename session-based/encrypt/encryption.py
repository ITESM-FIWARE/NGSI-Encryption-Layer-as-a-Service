...
from_address = "email address getmail"
...
smtp.starttls()
smtp.ehlo()
#smtp.login(from_address,os.environ['P_SADEWDA_E'])# Iniciar sesion en el servidor    
...
def sendEMAIL(email, asunto, message):
    try:
    	...
    	smtp.login(from_address,os.environ['P_SADEWDA_E'])
...
def encrypt():
    if "user" in session:
        ...
        for data in collection.find({"user":user},{"email":1,"_id":0}):
            data = data #Obtener datos del usuario
        email = data.get('email') # Obtener email del diccionario
        try:
            jsonfile = request.form['json'] # Obtener JSON del formulario
        except: 
            return jsonify({'message':'Enter json'})
        try:
            decoder = json.JSONDecoder(object_pairs_hook=OrderedDict) # OrderecDict para mantener el orden del JSON de entrada
            jsonfile = decoder.decode(jsonfile)
        except:
            return jsonify({'message':'JSON not valid'})
        idEntity = jsonfile.get('id')
        typeEntity = jsonfile.get('type')
        with open('inencrypt.json', 'w') as file: # Guardar JSON en in.json
            json.dump(jsonfile, file, indent=4)
        ...
        message = ("TYPE: "+typeEntity+"\n"+"User: "+user+"\n"+"Keys:"+"\n"+keys)
        returnEmailMetod = sendEMAIL(email, asunto, message)
        returnEmailMetod = [returnEmailMetod]
        if 'error' in returnEmailMetod:
